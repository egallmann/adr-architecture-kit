...
rendered_hash: 80e050ec02124f4cd045418bb59171253ec798db4b9f0287a0229f2d1c56abb0
-->
...
| DEC-0106 | Keep v1.0 as the stable backward-compatible compatibility line, v1.2 migratable, and v1.3 as a separate provisional authoring line | — |
...
| --- | --- | --- | --- |
| INV-0094 | Every admitted v1.3 entity's canonical machine identity is a lowercase RFC 9562 UUIDv7. | MUST / design | automated |
| INV-0009 | A canonical entity UUID is immutable for the entity's lifetime. | MUST / design | automated |
| INV-0010 | `alias_id`, `alias_name`, and derived `alias_ref` are human-recognition surfaces, not canonical machine identity. | MUST / design | automated |
| INV-0011 | Canonical authored entity references and normalized relationship endpoints use UUIDs. | MUST / design | automated |
| INV-0012 | An authorized alias change does not change canonical UUID identity. | MUST / design | automated |
| INV-0013 | An alias change never rewrites canonical UUID references or normalized UUID relationship endpoints; only derived… | MUST / design | automated |
| INV-0062 | The derived logical URI is a resolver key and is not entity identity. | MUST / design | automated |
| INV-0063 | Every normalized identity-bearing entity exposes explicit type; a polymorphic authored record authors type when… | MUST / design | automated |
| INV-0084 | `alias_ref` is always derived from `alias_id` and `alias_name` and is never canonical authored data. | MUST / design | automated |
| INV-0085 | `alias_name` provides recognition metadata and never defines or substitutes for architectural meaning. | MUST / design | automated |
| INV-0086 | Derived `created_at` denotes the beginning of the v1.3 identity record at UUIDv7 mint time, not historical… | MUST / design | automated |
| INV-0087 | Derived `created_at` is decoded from the UUIDv7 Unix-millisecond timestamp, leaving no separately authored value to… | MUST / design | automated |
| INV-0088 | The constraint that canonical `updated_at` cannot precede `created_at` is deferred with canonical `updated_at` to… | MAY / design | manual |
| INV-0089 | Regenerating derived artifacts never mutates canonical identity or v1.3 identity time semantics. | MUST / design | automated |
| INV-0090 | Two semantically distinct records claiming one UUID fail closed; ADR Kit does not choose a keeper or mint a… | MUST / design | automated |
| INV-0091 | A duplicate local `alias_id` across distinct UUIDs is a governed allocation conflict; without an admitted incumbent… | MUST / design | automated |
| INV-0092 | Alias recovery or reallocation never replaces an already valid canonical UUID. | MUST / design | automated |
| INV-0093 | Alias recovery or reallocation never rewrites canonical UUID relationship endpoints. | MUST / design | automated |
...
A canonical entity UUID is immutable for the entity's lifetime.
...
`alias_id`, `alias_name`, and derived `alias_ref` are human-recognition surfaces, not canonical machine identity.
...
Canonical authored entity references and normalized relationship endpoints use UUIDs.
...
An authorized alias change does not change canonical UUID identity.
...
An alias change never rewrites canonical UUID references or normalized UUID relationship endpoints; only derived alias presentation regenerates.
...
The derived logical URI is a resolver key and is not entity identity.
...
Every normalized identity-bearing entity exposes explicit type; a polymorphic authored record authors type when enclosing context is insufficient, while monomorphic authored schema position remains canonical type authority and `alias_id` alone never determines type.
...
`alias_ref` is always derived from `alias_id` and `alias_name` and is never canonical authored data.
...
`alias_name` provides recognition metadata and never defines or substitutes for architectural meaning.
...
Derived `created_at` denotes the beginning of the v1.3 identity record at UUIDv7 mint time, not historical architecture creation time.
...
Derived `created_at` is decoded from the UUIDv7 Unix-millisecond timestamp, leaving no separately authored value to reconcile.
...
The constraint that canonical `updated_at` cannot precede `created_at` is deferred with canonical `updated_at` to transactional-authoring governance and is not a v1.3 identity invariant.
...
Regenerating derived artifacts never mutates canonical identity or v1.3 identity time semantics.
...
Two semantically distinct records claiming one UUID fail closed; ADR Kit does not choose a keeper or mint a replacement automatically.
...
A duplicate local `alias_id` across distinct UUIDs is a governed allocation conflict; without an admitted incumbent it fails admission pending reviewed allocation, while cross-namespace overlap is not a collision.
...
Alias recovery or reallocation never replaces an already valid canonical UUID.
...
Alias recovery or reallocation never rewrites canonical UUID relationship endpoints.
...
**Context:** Classification: deferred gap. D-12/I-13 are recorded as deferred children and must not be treated as active v1.3 identity constraints.
**Impact:** medium
**Blocking:** false
...
Promoted from Design Journal via adr_kit.api promotion provider.
