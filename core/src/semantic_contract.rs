...
fn role_is_conformance(role: &str) -> bool {
    matches!(
        role,
        "normative-conformance" | "interpretation-conformance" | "normalized-model-conformance"
    )
}
...
fn canonical_number(value: &serde_json::Number) -> Result<String, String> {
    ...
    if !raw.contains('.') && !raw.contains('e') && !raw.contains('E') {
        ...
        if integer.abs() > MAX_SAFE_INTEGER {
            return Err(
                "unsafe integer input; values beyond IEEE-754 safe range are rejected".into(),
            );
        }
    }
    let number = value
...
pub(crate) fn canonicalize_value(value: &Json, output: &mut String) -> Result<(), String> {
    match value {
...
fn parse_definition(
    ...
    for key in definition.keys() {
        if !allowed.contains(&key.as_str()) {
            ...
            add_diagnostic(
                diagnostics,
                code,
                format!("field is outside the immutable definition: {key}"),
                Some(key.clone()),
            );
        }
    }
    if definition
        .get("semanticContractFamily")
        .and_then(Json::as_str)
        .filter(|value| is_family(value))
        .is_none()
    {
        add_diagnostic(
            diagnostics,
            "semantic_contract.invalid_family",
            "family must match the approved lower-case grammar",
            Some("semanticContractFamily".into()),
        );
    }
    if definition
        .get("semanticContractVersion")
        ...
        .is_none()
    {
        add_diagnostic(
            diagnostics,
            "semantic_contract.invalid_version",
            "version must be an exact major.minor value",
            Some("semanticContractVersion".into()),
        );
    }
    if definition.get("fingerprintScheme").and_then(Json::as_str) != Some(SCF_SCHEME) {
        add_diagnostic(
            diagnostics,
            "semantic_contract.invalid_fingerprint_scheme",
            format!("fingerprintScheme must be {SCF_SCHEME}"),
            Some("fingerprintScheme".into()),
        );
    }
    if require_fingerprint && !definition.contains_key("semanticContractFingerprint") {
        add_diagnostic(
            diagnostics,
            "semantic_contract.missing_fingerprint",
            "an immutable definition must declare semanticContractFingerprint",
            Some("semanticContractFingerprint".into()),
        );
    }
    if let Some(value) = definition.get("semanticContractFingerprint") {
        if value.as_str().filter(|value| is_scf(value)).is_none() {
            add_diagnostic(
                diagnostics,
                "semantic_contract.invalid_fingerprint",
                "semanticContractFingerprint must match ^scf:v1:sha256:[0-9a-f]{64}$",
                Some("semanticContractFingerprint".into()),
            );
        }
    }
    let Some(raw_manifest) = definition.get("resourceManifest").and_then(Json::as_array) else {
        add_diagnostic(
            diagnostics,
            "semantic_contract.missing_resource_manifest",
            "resourceManifest must be an array",
            Some("resourceManifest".into()),
        );
        return None;
    };
    if raw_manifest.is_empty() {
        add_diagnostic(
            diagnostics,
            "semantic_contract.empty_resource_manifest",
            "resourceManifest must not be empty",
            Some("resourceManifest".into()),
        );
    }
    ...
    for (index, raw) in raw_manifest.iter().enumerate() {
        ...
        let Some(item) = raw.as_object() else {
            add_diagnostic(
                diagnostics,
                "semantic_contract.invalid_resource",
                "resource manifest entry must be an object",
                Some(path),
            );
            continue;
        };
        for key in item.keys() {
            ...
            .contains(&key.as_str())
            {
                add_diagnostic(
                    diagnostics,
                    "semantic_contract.unknown_resource_field",
                    format!("field is not allowed in a resource manifest entry: {key}"),
        ...
        if !canonical_key(&key) {
            add_diagnostic(
                diagnostics,
                "semantic_contract.invalid_resource_key",
                "resource key is not canonical",
                Some(format!("{path}.canonicalResourceKey")),
            );
        }
        if !keys.insert(key.clone()) {
            add_diagnostic(
                diagnostics,
                "semantic_contract.duplicate_resource_key",
                "resource key occurs more than once",
                Some(format!("{path}.canonicalResourceKey")),
            );
        }
        let digest = item
        ...
        if !is_sha256(&digest) {
            add_diagnostic(
                diagnostics,
                "semantic_contract.invalid_resource_digest",
                "resource digest must be sha256:<64 lowercase hex characters>",
                Some(format!("{path}.contentDigest")),
            );
        }
        let role = item
            .get("role")
        ...
        if !role_is_valid(&role) {
            add_diagnostic(
                diagnostics,
                "semantic_contract.invalid_resource_role",
                "resource role is not in the governed vocabulary",
                Some(format!("{path}.role")),
            );
        }
        let Some(raw_dependencies) = item.get("dependencies").and_then(Json::as_array) else {
            add_diagnostic(
                diagnostics,
                "semantic_contract.missing_dependencies",
                "dependencies must be explicit, including when empty",
                Some(format!("{path}.dependencies")),
            );
        ...
        for (dependency_index, raw_dependency) in raw_dependencies.iter().enumerate() {
            ...
            let Some(dependency) = raw_dependency.as_object() else {
                add_diagnostic(
                    diagnostics,
                    "semantic_contract.invalid_dependency",
            ...
            for field in dependency.keys() {
                if !["canonicalResourceKey", "contentDigest"].contains(&field.as_str()) {
                    add_diagnostic(
                        diagnostics,
                        "semantic_contract.unknown_dependency_field",
                        format!("field is not allowed in a dependency: {field}"),
                        Some(format!("{dependency_path}.{field}")),
                    );
                }
            }
            ...
            if !canonical_key(&dependency_key) {
                add_diagnostic(
                    diagnostics,
                    "semantic_contract.invalid_dependency_key",
                    "dependency resource key is not canonical",
                    Some(format!("{dependency_path}.canonicalResourceKey")),
                );
            }
            if !is_sha256(&dependency_digest) {
                add_diagnostic(
                    diagnostics,
                    "semantic_contract.invalid_dependency_digest",
                    "dependency digest is invalid",
                    Some(format!("{dependency_path}.contentDigest")),
                );
            }
            if !dependency_keys.insert(dependency_key.clone()) {
                add_diagnostic(
                    diagnostics,
                    "semantic_contract.duplicate_dependency",
                    "a resource dependency may appear only once",
                    Some(format!("{dependency_path}.canonicalResourceKey")),
                );
            }
    ...
    for entry in &entries {
        for (dependency_key, dependency_digest) in &entry.dependencies {
            match by_key.get(dependency_key) {
                None => add_diagnostic(
                    diagnostics,
                    "semantic_contract.dangling_dependency",
                    format!("dependency does not exist: {dependency_key}"),
                    Some(entry.key.clone()),
                ),
                Some(target) if target.digest != *dependency_digest => add_diagnostic(
                    diagnostics,
                    "semantic_contract.conflicting_dependency_digest",
                    format!("dependency digest does not match resource manifest: {dependency_key}"),
                    Some(entry.key.clone()),
                ),
                Some(_) => {}
            }
    ...
    let Some(raw_frozen) = definition
        .get("frozenNormativeConformanceResources")
        .and_then(Json::as_array)
    else {
        add_diagnostic(
            diagnostics,
            "semantic_contract.missing_frozen_resources",
            "frozenNormativeConformanceResources must be an array",
            Some("frozenNormativeConformanceResources".into()),
        );
        return Some(entries);
    };
    if raw_frozen.is_empty() {
        add_diagnostic(
            diagnostics,
            "semantic_contract.empty_frozen_resources",
            "frozenNormativeConformanceResources must not be empty",
            Some("frozenNormativeConformanceResources".into()),
        );
    }
    ...
    for (index, value) in raw_frozen.iter().enumerate() {
        let Some(key) = value.as_str() else {
            add_diagnostic(
                diagnostics,
                "semantic_contract.invalid_frozen_resource",
                "frozen resource key must be a string",
                Some(format!("frozenNormativeConformanceResources[{index}]")),
            );
            continue;
        };
        if !frozen.insert(key.to_owned()) {
            add_diagnostic(
                diagnostics,
                "semantic_contract.duplicate_frozen_resource",
                "frozen resource key occurs more than once",
                Some(key.to_owned()),
            );
        }
        match by_key.get(key) {
            None => add_diagnostic(
                diagnostics,
                "semantic_contract.dangling_frozen_resource",
                "frozen resource is absent from resourceManifest",
                Some(key.to_owned()),
            ),
            Some(entry) if !role_is_conformance(&entry.role) => add_diagnostic(
                diagnostics,
                "semantic_contract.invalid_frozen_resource_role",
                "frozen resource role must be a declared conformance role",
                Some(key.to_owned()),
            ),
            Some(_) => {}
        }
...
fn detect_cycle(
    ...
    if !visiting.insert(key.to_owned()) {
        add_diagnostic(
            diagnostics,
            "semantic_contract.dependency_cycle",
...
fn normalized_definition(definition: &BTreeMap<String, Json>) -> BTreeMap<String, Json> {
    ...
    if let Some(Json::Array(manifest)) = normalized.get_mut("resourceManifest") {
        for entry in manifest.iter_mut() {
            if let Json::Object(values) = entry {
                if let Some(Json::Array(dependencies)) = values.get_mut("dependencies") {
    ...
    if let Some(Json::Array(frozen)) = normalized.get_mut("frozenNormativeConformanceResources") {
...
fn definition_preimage(definition: &BTreeMap<String, Json>) -> Result<String, String> {
    let preimage = object([
        ...
        (
            "definition".into(),
            Json::Object(normalized_definition(definition)),
        ),
    ]);
    let mut canonical = String::new();
    canonicalize_value(&preimage, &mut canonical)?;
    ...
    if require_declared {
            ...
            .and_then(Json::as_str)
        {
            ...
            add_diagnostic(
                diagnostics,
                "semantic_contract.missing_fingerprint",
                "verification requires a declared semanticContractFingerprint",
                Some("semanticContractFingerprint".into()),
            );
        }
    }
...
fn relative_resource_key(
    ...
    if reference.starts_with("//") || reference.contains("://") || reference.starts_with("urn:") {
        return Err((
            "semantic_contract.live_external_reference".into(),
            "remote resource references are not permitted in a semantic closure".into(),
        ));
    }
    if location.starts_with('/') {
        return Err((
            "semantic_contract.absolute_resource_reference".into(),
            "absolute resource references cannot be resolved from a semantic resource".into(),
        ));
    }
    ...
    for part in location.split('/') {
        ...
        if part == ".." {
            if parts.pop().is_none() {
                return Err((
                    "semantic_contract.resource_reference_escape".into(),
                    "relative resource reference escapes the canonical resource namespace".into(),
                ));
            }
        } else {
    ...
    let Some(last) = parts.last_mut() else {
        return Err((
            "semantic_contract.invalid_resource_reference".into(),
            "relative resource reference resolved to an empty key".into(),
        ));
    };
    ...
    if !canonical_key(&key) {
        return Err((
            "semantic_contract.invalid_resource_reference".into(),
            "relative resource reference did not resolve to a canonical key".into(),
        ));
    }
    Ok(Some(key))
}
        ...
        Json::Object(values) => {
            if let Some(reference) = values.get("$ref") {
                let Some(reference) = reference.as_str() else {
                    add_diagnostic(
                        diagnostics,
                        "semantic_contract.invalid_resource_reference",
                ...
                match relative_resource_key(resource_key, reference) {
                    ...
                    Ok(None) => {}
                    Err((code, message)) => {
                        add_diagnostic(diagnostics, &code, message, Some(path.to_owned()))
                    }
                }
            }
...
fn collect_explicit_semantic_dependencies(
    resource_key: &str,
    value: &Json,
    diagnostics: &mut Vec<Json>,
) -> BTreeSet<String> {
    let Some(raw) = value
        .as_object()
        .and_then(|object| object.get("semanticDependencies"))
    else {
        return BTreeSet::new();
    };
    let Some(values) = raw.as_array() else {
        add_diagnostic(
            diagnostics,
            "semantic_contract.invalid_semantic_dependencies",
            "semanticDependencies must be an array of canonical resource keys",
            Some(resource_key.to_owned()),
        );
        return BTreeSet::new();
    };
    ...
    for (index, dependency) in values.iter().enumerate() {
        ...
        let Some(key) = dependency.as_str() else {
            add_diagnostic(
                diagnostics,
                "semantic_contract.invalid_semantic_dependency",
                "semantic dependency must be a string",
                Some(path),
            );
            continue;
        };
        if !canonical_key(key) {
            add_diagnostic(
                diagnostics,
                "semantic_contract.invalid_semantic_dependency",
                "semantic dependency key is not canonical",
                Some(path),
            );
        }
...
fn validate_resource_contents(
    ...
    let Some(resources) = resources else {
        add_diagnostic(
            diagnostics,
            "semantic_contract.missing_resource_contents",
            "resource closure requires explicit resource contents",
            Some("resources".into()),
        );
        return;
    };
    if resources.is_empty() {
        add_diagnostic(
            diagnostics,
            "semantic_contract.empty_resource_contents",
            "resource closure contents must not be empty",
            Some("resources".into()),
        );
    }
    ...
    for (index, raw) in resources.iter().enumerate() {
        ...
        let Some(resource) = raw.as_object() else {
            add_diagnostic(
                diagnostics,
                "semantic_contract.invalid_resource_content",
                "resource content entry must be an object",
                Some(path),
            );
            continue;
        };
        for key in resource.keys() {
            if !["canonicalResourceKey", "content"].contains(&key.as_str()) {
                add_diagnostic(
                    diagnostics,
                    "semantic_contract.unknown_resource_content_field",
                    format!("field is not allowed in supplied resource content: {key}"),
                    Some(format!("{path}.{key}")),
                );
            }
        }
        ...
        if !canonical_key(&key) {
            add_diagnostic(
                diagnostics,
                "semantic_contract.invalid_supplied_resource_key",
                "supplied resource key is not canonical",
                Some(format!("{path}.canonicalResourceKey")),
            );
        }
        ...
        if let Some(previous) = supplied.insert(key.clone(), content.clone()) {
            ...
            let same = canonicalize_value(&previous, &mut previous_json).is_ok()
                && canonicalize_value(&content, &mut current_json).is_ok()
                && previous_json == current_json;
            add_diagnostic(
                diagnostics,
                if same {
                    "semantic_contract.duplicate_supplied_resource"
                } else {
                    "semantic_contract.conflicting_resource_definition"
                },
                "resource content key occurs more than once",
                Some(key),
            );
        }
    }
    for entry in entries {
        match supplied.get(&entry.key) {
            None => add_diagnostic(
                diagnostics,
                "semantic_contract.missing_resource_content",
                format!("missing content for resource: {}", entry.key),
                Some(entry.key.clone()),
            ),
            Some(content) => {
                let mut canonical = String::new();
                if let Err(error) = canonicalize_value(content, &mut canonical) {
                    ...
                    add_diagnostic(
                        ...
                        format!("content digest does not match manifest for {}", entry.key),
                        Some(entry.key.clone()),
                    );
                }
                ...
                discovered.extend(collect_explicit_semantic_dependencies(
                    &entry.key,
                    content,
                    diagnostics,
                ));
                ...
                for key in discovered.difference(&declared) {
                    add_diagnostic(
                        diagnostics,
                        "semantic_contract.undeclared_dependency",
                        format!("resource reference is not declared in the manifest: {key}"),
                        Some(entry.key.clone()),
                    );
                }
                for key in declared.difference(&discovered) {
                    add_diagnostic(
                        diagnostics,
                        "semantic_contract.unjustified_dependency",
                        format!("manifest dependency is not justified by the resource: {key}"),
                        Some(entry.key.clone()),
                    );
                }
                for key in discovered {
                    if !entries.iter().any(|candidate| candidate.key == key) {
                        add_diagnostic(
                            diagnostics,
                            "semantic_contract.unresolved_resource_dependency",
                            format!("resource dependency is not present in the manifest: {key}"),
                            Some(entry.key.clone()),
                        );
                    }
                }
    ...
    for key in supplied.keys() {
        if !entries.iter().any(|entry| entry.key == *key) {
            add_diagnostic(
                diagnostics,
                "semantic_contract.unlisted_resource",
                format!("resource content is not listed in the manifest: {key}"),
                Some(key.clone()),
            );
        }
    }
...
pub fn canonicalize(request: &Json) -> Json {
    ...
    if root.contains_key("value") && root.contains_key("value_json") {
        return canonical_failure(
            "canonicalize_semantic_json",
            vec![diagnostic(
                "semantic_contract.ambiguous_value",
                "value and value_json are mutually exclusive",
                None,
            )],
        );
    }
        ...
        parsed_value = match serde_json::from_str::<Json>(raw) {
            ...
            Err(error) => {
                return canonical_failure(
                    "canonicalize_semantic_json",
                    vec![diagnostic(
                        "semantic_contract.invalid_json",
                        error.to_string(),
                        Some("value_json".into()),
                    )],
                )
            }
        };
    ...
    match canonicalize_value(value, &mut canonical) {
        ...
        Err(error) => canonical_failure(
            "canonicalize_semantic_json",
            vec![diagnostic(
                "semantic_contract.unsafe_number",
                error,
                Some("value".into()),
            )],
        ),
...
pub fn fingerprint(request: &Json) -> Json {
    ...
    if !matches!(mode, "calculate" | "verify") {
        return super::invalid("mode must be calculate or verify");
    }
    let mut diagnostics = Vec::new();
    let Some(canonical) =
        validate_declared_fingerprint(definition, mode == "verify", &mut diagnostics)
    else {
    ...
    let mut result = canonical_result(
        "fingerprint_semantic_contract",
        canonical,
        fingerprint.clone(),
    );
    if let Json::Object(ref mut values) = result {
        values.insert("mode".into(), string(mode));
...
pub fn validate_closure(request: &Json) -> Json {
    ...
    let mut result = match simple_result(
...
pub fn compose_set(request: &Json) -> Json {
    ...
    if raw_contracts.is_empty() {
        add_diagnostic(
            &mut diagnostics,
            "semantic_contract.empty_set",
            "a semantic contract set must contain at least one member",
            Some("contracts".into()),
        );
    }
...
mod tests {
    ...
    fn ecmascript_number_spelling_matches_the_shared_known_answer() {
        ...
        let Json::Object(values) = &value else {
            panic!("known-answer JSON must be an object")
        };
        ...
        let Json::Number(a) = values.get("a").expect("a is present") else {
            panic!("a must be a number")
        };
        let Json::Number(m) = values.get("m").expect("m is present") else {
            panic!("m must be a number")
        };
        assert_eq!(
            a.as_f64().expect("finite number").to_bits(),
            4734372014072354133
        );
        assert_eq!(
            m.as_f64().expect("finite number").to_bits(),
            4876203697187506177
        );
        let mut canonical = String::new();
        canonicalize_value(&value, &mut canonical)
            .expect("known-answer numbers are in the supported domain");
        assert_eq!(
            canonical,
            r#"{"a":333333333.3333333,"b":1e+30,"c":4.5,"d":0.002,"e":1e-27,"f":0.000001,"g":1e-7,"h":100000000000000000000,"i":1e+21,"j":0,"k":5e-324,"l":1.7976931348623157e+308,"m":1000000000000000100}"#
        );
    }
}
