...
class SystemOverviewGenerator:
    ...
    def __init__(self, template_dir: Path | None = None):
        ...
        # Resolve the repository being documented from the CLI working directory.
    ...
    def _workspace_highlights(self, project_name: str) -> list[str]:
        """Surface key workspace-scope ADRs when generating ste-runtime overview."""
        if project_name != "ste-runtime":
            return []
        ...
        for adr in data.get("adrs", []):
            if not isinstance(adr, dict):
                continue
            aid = adr.get("id")
            if aid in ("ADR-L-0009", "ADR-L-0010"):
                title = adr.get("title", aid)
                lines.append(f"- **{aid}**: {title}")
        lines.append(
            "- **INV-0014**: Scope is a workspace (declared in ADR-L-0009; supersedes INV-0001)."
        )
        return lines
    ...
    def build_context(self) -> Dict[str, Any]:
        ...
        project_block = meta.get("project") if isinstance(meta.get("project"), dict) else {}
        project_name = project_block.get("name") or "adr-architecture-kit"

        if project_name == "ste-runtime":
            authority_impl = "code under src/ as implementation"
            system_purpose_lead = (
                "`ste-runtime` implements STE runtime workflows including RECON, RSS, and MCP integration."
            )
            system_purpose_bullets = [
                "TypeScript sources under `src/` with a phased RECON reconciliation pipeline",
                "NPM package entrypoints (`ste`, `recon`, `rss`) defined in `package.json`",
                "Architecture authority under `adrs/` with regeneration via the `adr` CLI",
                "Automated quality gates via Vitest and ESLint (`npm run test`, `npm run lint`)",
            ]
            system_purpose_footer = (
                "Normative STE doctrine remains in `ste-spec` when that repository is present in your workspace."
            )
        else:
            authority_impl = "code under src/adr_kit/ as implementation"
            system_purpose_lead = (
            ...
            system_purpose_footer = "It does not define STE doctrine. That authority remains in `ste-spec`."

        workspace_highlights = self._workspace_highlights(project_name)
        ...
        return {
            ...
            "system_purpose_lead": system_purpose_lead,
            "system_purpose_bullets": system_purpose_bullets,
            "system_purpose_footer": system_purpose_footer,
            ...
            "cli_capabilities": [
                ...
                "`adr attribution generate-shim --language python|typescript`",
                "`adr audit-runtime`",
            ...
            "canonical_workflows": [
                ...
                OverviewRule("Generate ADR human projections", "Run `adr generate-adr-projection` (or compatibility alias `adr generate-rendered-docs`). Do not hand-edit files under `adrs/adr-projection/`."),
                OverviewRule("Validate generated documentation", "Run `adr validate-generated-docs` after regenerating manifest or ADR human projection artifacts. This does not validate `README.md` or `SYSTEM-OVERVIEW.md`."),
                ...
                OverviewRule(
                    "Inspect implementation linkage evidence",
                    "After refreshing discovery outputs (`adr generate-architecture-index`, `manifest`, etc.), run `adr attribution check` on representative `implementation-attribution-evidence.yaml` files (`--profile` selects greenfield vs brownfield rules). Use `adr attribution coverage` for a corpus-vs-evidence citation summary.",
                ),
    ...
    def declared_source_inputs(self, output_path: Path) -> list[Path | HashInput]:
        ...
        if manifest_yaml.is_file():
            inputs.append(HashInput("adrs/manifest.yaml", manifest_yaml.read_bytes()))
        return inputs
