"""Normalized architecture model v2.2."""
...
from .registries import RelationshipV22


class NormalizedArchitectureModelV22(BaseModel):
    ...
    def find_entity(self, entity_id: str) -> NormalizedEntityV22 | None:
        return next((entity for entity in self.entities if entity.id == entity_id), None)

    def find_entity_by_alias_id(self, alias_id: str) -> NormalizedEntityV22 | None:
        return next((entity for entity in self.entities if entity.alias_id == alias_id), None)

    def entities_by_type(self, entity_type: str) -> list[NormalizedEntityV22]:
