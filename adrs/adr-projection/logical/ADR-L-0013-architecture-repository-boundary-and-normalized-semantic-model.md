...
rendered_hash: 80d57783e8968a8ace258c92ece069e9d8814817b4eddbf348b596c900e045ca
-->
...
**Acceptance criteria**
- ArchitectureRepository loads compiled bundles and validates them at load time
- Consumers can retrieve a NormalizedArchitectureModel without reading raw YAML
- Unresolved records and provenance remain first-class in the semantic model
- Compiler-internal ArchModel is not required by normal consumer workflows
- Repository evolution remains the preferred in-process seam for graph access once implemented
- The v1.3 repository embodiment exposes model 2.0 UUID/alias/URI resolution
...
**Acceptance criteria**
- cross-language consumers bootstrap from `architecture-index.yaml`
- the required runtime ingestion subset is explicit and smaller than the full guaranteed contract family
- subset registries and `architecture-graph.yaml` remain additive
- `manifest.yaml` is used for discovery and freshness orientation, not semantic authority
- the legacy compatibility registry remains unavailable for new consumer dependencies
...
**Acceptance criteria**
- the facade exposes only explicitly authorized supported public symbols for the current API contract, including additive promotion-provider operations once separately authorized
- validation and compilation return immutable public results without compiler-internal types
- preview and write compilation return byte-equivalent artifacts for identical pinned inputs
- repository opening returns the existing loaded ArchitectureRepository contract
- capability discovery is deterministic, local, resource-backed, and versioned
- schema v1.0, provisional v1.1 treatment, CLI behavior, and generated bytes remain compatible
- When model 2.0 compatibility adapters are embodied, they remain on the supported facade
