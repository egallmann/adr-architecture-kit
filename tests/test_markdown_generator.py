...
class TestMarkdownGeneration:
    ...
    def test_render_complete_logical_adr(self, parser, generator):
        """Context-free MarkdownGenerator is legacy; Projection v3 is scope-aware."""
        ...
        assert "## Context" in markdown
        assert str(adr.context).splitlines()[0] in markdown
        assert "## Capabilities" not in markdown
        assert "## Architectural Decisions" not in markdown

        scope = ProjectScopeResolver().resolve()
        artifact_path = Path(
            "adrs/adr-projection/logical/"
            "ADR-L-0001-ste-compliant-machine-verifiable-architecture-decision-record-system.md"
        )
        compiler_markdown, _inputs = generator.render_existing_artifact(artifact_path, scope)
        assert "## Capabilities" in compiler_markdown
        assert "## Invariants" in compiler_markdown
        assert "## Architectural Decisions" in compiler_markdown
        assert "CAP-0001" in compiler_markdown
        assert "INV-0001" in compiler_markdown
